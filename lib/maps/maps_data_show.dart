...
import 'package:okstitch/maps/key.dart';
import 'package:okstitch/maps/request_api.dart';
import 'package:okstitch/maps/user_destination.dart';
...
class _MapsDataShowState extends State<MapsDataShow> {
                    ...
                    Padding(
                      ...
                      child: TextButton(onPressed: () {
                        Navigator.pop(context);
                          ...
                          child: const Text(
                            "Close", style: TextStyle(fontSize: 18),)),
                    )
                  ]),
              Padding(
  ...
  Future<void> _placeApiRequest(String userkeyboardData, String key) async {
    UserData.originLatitude = widget.latitude;
    UserData.originLongitude = widget.longitude;
    if (userkeyboardData.length > 1) {
      String autoComplete =
          "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$userkeyboardData&location=${widget
          .latitude}%2C${widget.longitude}&radius=50&key=$key";
...
class PlacePrediction {
  ...
  PlacePrediction(
      {required this.secondaryText, required this.mainText, required this.placeId});
